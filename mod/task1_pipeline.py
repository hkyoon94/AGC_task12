import time, json, jsbeautifier, sys;
...
from inference.task1_routines import collect_high_cos_sim, qa_model_or_similarity, chunk_n_by_sim, multi_filter_and_cos, \
    direct_fill
from inference.task2_routines import solve_task2_A, solve_task2_B
...
def run(task_no_1_q, task_no_1_ans, doc_path):
    pipeline_arguments = {
        'task_no_1_q': task_no_1_q,
        'task_no_1_ans': task_no_1_ans,
        'doc_path': doc_path,
    ...
    return base_pipe.task_no_1_ans
...
class Pipeline:
    def __init__(self,
        ...
        # 파이프라인 인수 정리
        self.task_no_1_q = task_no_1_q
        self.task_no_1_ans = task_no_1_ans
        ...
        self.generate_task1_context = context_generator_task1['method']
        self.task1_solver = task1_solver['method']
        ...
        for prob_id, prob in enumerate(self.task_no_1_q):  # 지정한 문제들을 따라 순회
            ...
            self.answer_form = self.task_no_1_ans[prob_id]['answer']  # answer form

            if is_local:
                # 데이터 repo 경로 설정 (xml_path 지정)
                sampledata_dir = os.path.join(data_path, "samples")  # 대회 샘플 데이터 디렉토리 경로
            ...
            # 문제 해결
            try:
                query_contexts = self.generate_task1_context(self,
                                                            corpus=corpus,
                                                            query=self.prob_query,
                                                            **context_generator_task1['kwargs'])
                inferred_answer = self.task1_solver(self,
                                                corpus=corpus,
                                                context=query_contexts,
                                                query=self.prob_query,
                                                answer_form=self.answer_form,
                                                **task1_solver['kwargs'])
                self.task_no_1_ans[prob_id]['answer'] = inferred_answer
            except:
                pass

            # !-----------------------------------------------------------------------------------------#
        #     self.record_prob_time()

        #     # 채점 루틴 (임시)
        #     if self.task_no == '1': # 유형 1 채점
        #         self.check_task1_answer(inferred_answer)

        #     elif self.task_no == '2': # 유형 2 채점
        #         self.check_task2_answer(inferred_answer)
